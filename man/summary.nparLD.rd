\name{summary.nparLD}
\alias{summary}
\alias{summary.nparLD}
\title{Object Summary for nparLD}
\description{
This function prints out a summary of the object returned by the function \code{\link{nparLD}}. The summary includes the model and the corresponding formula, the relative treatment effect (RTE), the Wald-type statistic (WTS), and the ANOVA-type statistic (ATS).
}

\usage{
\method{summary}{nparLD}(object, \ldots)
}

\arguments{
  \item{object}{an \code{nparLD} class object generated by \code{\link{nparLD}} (see examples).}
